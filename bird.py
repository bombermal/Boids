...
class bird():
    def __init__(self, repulsion, attraction, width, height, screen, groupId, leader=False):
        self.screen = screen
        ...
        self.leader = leader
        self.groupId = np.random.choice(range(groupId)) 
        self.inAGroup = False
        self.maxVel = 2
        self.maxForce = .5
    ...
    def seek(self, birdsList):
        xPos, yPos = self.position
        for others in birdsList:
            othersX, othersY = others.position
            if others != self:
                distance = np.hypot(xPos - othersX, yPos - othersY)
                if distance < self.repulsion:
                    if others.groupId == self.groupId and ( others.leader or others.inAGroup ):
                        self.velocity = others.velocity
                        self.inAGroup = True
                        
                else:
                    if others.groupId == self.groupId and others.leader:
                        desired_velocity = fc.normalise(self.position - others.position) * self.maxVel
